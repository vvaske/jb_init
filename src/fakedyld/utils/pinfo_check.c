...
void pinfo_check(struct paleinfo* pinfo_p) {
    ...
    /* flags that that are REQUIRED for booting */
    if ((pinfo_p->flags & (
        palerain_option_bind_mount |
        palerain_option_overlay
    )) != (
        palerain_option_bind_mount |
        palerain_option_overlay
    )) {
        LOG("bind mount and overlay flags are required for this ramdisk");
        spin();
    }
    ...
    struct {
        ...
        { palerain_option_setup_rootful | palerain_option_setup_partial_root, "Cannot setup a partial fakefs without also setting up fakefs" },
